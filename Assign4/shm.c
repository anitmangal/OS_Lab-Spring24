...
main()
{
	...
	*/
	shmid = shmget(IPC_PRIVATE, 2*sizeof(int), 0777|IPC_CREAT);
		...
		// key can refer to some shared memory segment (give it a name)
		// size is the size of the shared memory segment
		// flag is the permission flag for the segment. IPC_CREAT : create a new segment if it doesn't exist, otherwise return the id of the existing segment. IPC_EXCL : return error if the segment already exists.
	...
	*/
	if (fork() == 0) {
		...
		// shmaddr : address at which the segment is to be attached. If NULL, the system chooses a suitable (unused) address.
		...
		shmdt(b); // 0 if succesful, -1 if error
		...
		*/
		...
		shmctl(shmid, IPC_RMID, 0);		// If not deleted, the shared memory will remain in the system even after the program exits. Use "ipcs -m" to find the shared memory segments in the system.
		// int shmctl(int shmid, int cmd, struct shmid_ds *buf);
		// cmd : IPC_RMID : delete the segment, IPC_STAT : get the current status of the segment, IPC_SET : set the 	status of the segment, IPC_INFO : get the information about the shared memory system
		// buf : pointer to a shmid_ds structure. Used for IPC_STAT and IPC_SET.
	}
}
